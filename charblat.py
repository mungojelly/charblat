...
PROGRAM_LENGTH = 30
MAX_STACK_LENGTH = 30
MAX_INSTRUCTION_STACK_LENGTH = 50
...
def run_instruction(program, context, energy):
    ...
    if instruction == 'put_a':
        context.pad.addch(context.y, context.x, ord('a'))
    if instruction == 'put_b':
        context.pad.addch(context.y, context.x, ord('b'))
    if instruction == 'put_c':
        context.pad.addch(context.y, context.x, ord('c'))
    if instruction == 'put_e':
        context.pad.addch(context.y, context.x, ord('e'))
    if instruction == 'put_i':
        context.pad.addch(context.y, context.x, ord('i'))
    if instruction == 'put_k':
        context.pad.addch(context.y, context.x, ord('k'))
    if instruction == 'put_l':
        context.pad.addch(context.y, context.x, ord('l'))
    if instruction == 'put_m':
        context.pad.addch(context.y, context.x, ord('m'))
    if instruction == 'push':
    ...
    if instruction == 'turn_a_to_d':
        if contents == b'a':
            context.pad.addch(context.y, context.x, ord('d'))
    if instruction == 'restart_if_e':
        if contents == b'e':
            context.current_instruction = -1
    if instruction == 'check_if_stack':
        if len(context.stack) == 0:
            context.pad.addch(context.y, context.x, ord('f'))
        else:
            context.pad.addch(context.y, context.x, ord('g'))
    ...
    if instruction == 'h_if_low_energy':
        if energy < LOW_ENERGY:
            context.pad.addch(context.y, context.x, ord('h'))
    ...
    if instruction == 'i_to_j':
        if contents == b'i':
            context.pad.addch(context.y, context.x, ord('j'))
...
def run_program(program, context, energy):
    while energy > 0:
        energy = energy - 1
        context = run_instruction(program, context, energy)
